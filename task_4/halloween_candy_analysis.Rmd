...
boing_boing_candy_2015_to_2017_clean %>% 
  mutate(review = case_when(
              str_detect(review, "JOY") ~ 1,
              str_detect(country, "DESPAIR") ~ -1,
              str_detect(country, "MEH") ~ 0
  )
)
...
boing_boing_candy_2015_to_2017_clean %>% 
  group_by(candy_name) %>% 
 # filter(str_detect(review, "JOY")) %>% 
  summarise(rating_score = length(str_detect(review, "JOY")) - length(str_detect(review, "DESPAIR"))) 
#  slice_max(joy)
